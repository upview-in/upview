                    ...
                    @foreach ($permissionHelper->getGroups() as $index => $groupName)
                        <div class="tab-pane fade show overflow-auto @once active @endonce" id="tab{{ $index }}" role="tabpanel">
                            <div class="d-flex g-4">
                                ...
                                @foreach($permissionHelper->getPermissionsColumnsFromGroup($groupName) as $column)
                                    <div class="col-2 p-3 border-end border-bottom fs-6">
                                    ...
                                    @foreach($permissionHelper->getPermissionsColumnsFromGroup($groupName) as $column)
                                        @if(!empty($permissionHelper->getSlug($groupName, $moduleName, $column)))
                                            <div class="col-2 p-3 border-end border-bottom">
                                                <div class="form-check form-switch d-flex justify-content-center">
                                                    <input class="form-check-input permission-switch" type="checkbox" name="permissions[]" data-group-name="{{ $groupName }}" data-module-name="{{ $moduleName }}" data-column-name="{{ $column }}" value="{{ $permissionHelper->getSlug($groupName, $moduleName, $column) }}">
                                                </div>
                                            </div>
                                        @else
                                            <div class="col-2 p-3 border-end border-bottom">
