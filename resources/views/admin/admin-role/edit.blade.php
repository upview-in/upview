...
<li class="breadcrumb-item active" aria-current="page">{{ $adminRole->name }}</li>
@endsection
            ...
            <div class="card-body p-5">
                ...
                <div class="row g-3">
                    <div class="col-md-6 col-sm-6 col-12">
                        ...
                        <input type="text" class="form-control" id="name" name="name" placeholder="Name" value="{{ old('name') ?? $adminRole->name }}" required>
                        @error('name')
                    ...
                    <div class="col-md-6 col-sm-6 col-12">
                        ...
                        <input type="text" class="form-control" id="slug" name="slug" placeholder="slug" value="{{ old('slug') ?? $adminRole->slug }}" required>
                        @error('slug')
                        <span class="invalid-feedback d-block" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
    ...
    <div class="card border-top border-0 border-4 border-success">
        <form class="ajax-form" method="POST" action="{{ route('admin.adminRoles.update', $adminRole->id) }}">
            ...
            <div class="card-body p-5">
                <div class="card-title d-flex align-items-center">
                    <div class="d-flex align-items-center w-100">
                        <em class="bi bi-fingerprint me-2 font-22 text-success"></em>
                    ...
                    @foreach ($adminPermissionHelper->getGroups() as $index => $groupName)
                        <div class="tab-pane fade show overflow-auto @once active @endonce" id="tab{{ $index }}" role="tabpanel">
                            <div class="d-flex g-4">
                                ...
                                @foreach($adminPermissionHelper->getPermissionsColumnsFromGroup($groupName) as $index => $column)
                                    <div class="col-2 p-3 border-end border-bottom fs-6">
                                    ...
                                    @foreach($adminPermissionHelper->getPermissionsColumnsFromGroup($groupName) as $index => $column)
                                        @if(!empty($adminPermissionHelper->getSlug($groupName, $moduleName, $column)))
                                            <div class="col-2 p-3 border-end border-bottom">
                                                <div class="form-check form-switch d-flex justify-content-center">
                                                    <input class="form-check-input permission-switch" type="checkbox" name="permissions[]" data-group-name="{{ $groupName }}" data-module-name="{{ $moduleName }}" data-column-name="{{ $column }}" value="{{ $adminPermissionHelper->getSlug($groupName, $moduleName, $column) }}" {{ !$havePermissions->contains($adminPermissionHelper->getSlug($groupName, $moduleName, $column))?:'checked' }}>
                                                </div>
                                            </div>
                                        @else
                                            <div class="col-2 p-3 border-end border-bottom">
