        ...
        <div class="card-body p-5">
            <div class="card-title d-flex align-items-center">
                ...
                <div class="d-flex flex-shrink-1 pointer">
                    ...
                    <form class="ajax-form hidden" method="POST" action="{{ route('admin.adminPermissions.import') }}">
                        @csrf
                        <div class="input-group">
                            <input type="file" name="file" id="import-file" onchange="$(this).closest('form').submit();">
                        </div>
                    </form>
                    <a href="{{ route('admin.adminPermissions.export') }}" class="d-flex align-items-center ps-3 text-warning"><em class="bi bi-cloud-arrow-down me-1 font-22"></em> <strong>Export</strong></a>
                    <a href="{{ route('admin.adminPermissions.create') }}" class="d-flex align-items-center ps-5 text-primary"><em class="bi bi-plus-circle me-1 font-22"></em> <strong>Create</strong></a>
                </div>
            </div>
            <div class="row mt-3">
            ...
            <div class="table-responsive">
                <table class="table mb-0 align-middle">
                    ...
                    <tbody>
                        @if (!$adminPermissions->total())
                            <tr>
                                <td colspan="100" class="text-center pt-5 pb-5">Oops! No data found.</td>
                            </tr>
                        @endif

                        @foreach($adminPermissions as $userPermission)
                        <tr>
                            <th scope="row">
                                <form class="ajax-form" method="POST" action="{{ route('admin.adminPermissions.destroy', $userPermission->id) }}">
                                    @method('delete')
                                    @csrf
                                    <a href="{{ route('admin.adminPermissions.edit', $userPermission->id) }}" class="btn btn-sm p-1">
                            ...
                            <td>
                                <form class="ajax-form" method="POST" action="{{ route('admin.adminPermissions.update', $userPermission->id) }}">
                                    @method('patch')
                                    @csrf
                ...
                <div class="mt-4">
                    {{ $adminPermissions->withQueryString()->links() }}
                </div>
            </div>
        </div>
    </div>
